...
BLE-based localization in real-world care facilities confronts significant data quality challenges. BLE signals experience substantial fluctuations in RSSI values caused by multipath propagation and environmental interference [11, 12]. Care facilities present particularly complex deployment scenarios characterized by beacon placement constraints and architectural complexity [8]. These environments exhibit several documented challenges: signal instability arising from multipath effects and device heterogeneity [13], spatial sparsity with limited beacon coverage in certain areas [11], temporal irregularity with variable detection rates [14], and severe class imbalance where common areas receive significantly more visits than individual patient rooms [15]. Traditional machine learning methods applied to such real-world datasets demonstrate limited performance despite various optimization efforts [9, 10]. This performance plateau reveals a critical limitation: the independence assumption inherent in traditional approaches discards temporal dependencies in human movement patterns [16]. Caregivers follow continuous trajectories through physical space, where location at time $t$ strongly predicts location at time $t+1$, and transitions between rooms produce gradual shifts in beacon signal patterns [17]. Traditional methods treat each temporal window as an isolated classification problem, thereby discarding contextual information about movement trajectories, dwell times, and transition dynamics.
...
\bibitem{b19} Y. Zhuang, J. Yang, Y. Li, L. Qi, and N. El-Sheimy, ``Smartphone-Based Indoor Localization with Bluetooth Low Energy Beacons,'' \textit{Sensors}, vol. 16, no. 5, p. 596, 2016.

\bibitem{b20} J. Torres-Sospedra et al., ``UJIIndoorLoc: A New Multi-building and Multi-floor Database for WLAN Fingerprint-based Indoor Localization Problems,'' in \textit{Proc. Int. Conf. Indoor Positioning Indoor Navigation}, Busan, South Korea, 2014, pp. 261-270.

\bibitem{b21} E. Garcia and E. Inoue, ``Relabeling Techniques for BLE-Based Indoor Localization,'' \textit{Sensors}, vol. 24, no. 2, p. 319, 2024.
...
\bibitem{b26} Z. Niu, G. Zhong, and H. Yu, ``A Review on the Attention Mechanism of Deep Learning,'' \textit{Neurocomputing}, vol. 452, pp. 48-62, 2021.
