...
\author{
\IEEEauthorblockN{Author Name\textsuperscript{1}, Author Name\textsuperscript{2}, Author Name\textsuperscript{3}}
\IEEEauthorblockA{\textsuperscript{1,2,3}\textit{Department Name} \\
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{5th floor map.png}
...
\FOR{each direction $d \in \{\text{backward\_10},$ $\text{centered\_10}, \text{forward\_10},$ $\text{backward\_15}, \text{forward\_15},$ $\text{asymm\_past}, \text{asymm\_future}\}$}
    ...
    \FOR{each model $i$ ($i = 1$ to $5$)}
        \STATE $\text{probability}_i =$ 
        \STATE \hspace{1em} $\text{model}_i.\text{predict}(\text{sequences}_d)$
    \ENDFOR
    \STATE $\text{averaged\_probability}_d =$ 
    \STATE \hspace{1em} $\text{mean}(\text{probability}_1, \ldots, \text{probability}_5)$
\ENDFOR
\end{algorithmic}
\end{algorithm}
...
\FOR{timestamp $t$ with 7 directional probability distributions}
    \FOR{each direction $d$ ($d = 1$ to $7$)}
        \STATE $\text{confidence}_d =$ 
        \STATE \hspace{1em} $\max(\text{probability\_distribution}_d)$
        \STATE $\text{weighted\_vote}_d =$ 
        \STATE \hspace{1em} $\text{probability\_distribution}_d \times$
        \STATE \hspace{1em} $\text{confidence}_d$
    \ENDFOR
    \STATE $\text{final\_probability}_t =$ 
    \STATE \hspace{1em} $\sum(\text{weighted\_vote}_d) /$
    \STATE \hspace{1em} $\sum(\text{confidence}_d)$
    \STATE $\text{final\_prediction}_t =$ 
    \STATE \hspace{1em} $\arg\max(\text{final\_probability}_t)$
\ENDFOR
\end{algorithmic}
\end{algorithm}
...
\FOR{each timestamp $j \in [t-2, t-1, t, t+1, t+2]$}
    \STATE $\text{confidence}_j =$ 
    \STATE \hspace{1em} $\max(\text{probability\_distribution}_j)$
    \STATE $\text{weighted\_vote} \mathrel{+}=$ 
    \STATE \hspace{1em} $\text{probability\_distribution}_j \times$
    \STATE \hspace{1em} $\text{confidence}_j$
\ENDFOR
\STATE $\text{smoothed\_prediction}_t =$ 
\STATE \hspace{1em} $\arg\max(\sum \text{weighted\_vote})$
\end{algorithmic}
\end{algorithm}
...
\begin{table*}[!htbp]
\caption{Macro F1 Scores Across 4-Fold Cross-Validation}
\label{tab:results}
\centering
\small
\begin{tabular}{|l|l|c|c|c|c|c|}
\hline
\textbf{Approach} & \textbf{Description} & \textbf{Fold 1} & \textbf{Fold 2} & \textbf{Fold 3} & \textbf{Fold 4} & \textbf{Mean ± Std} \\
\hline
Baseline & 3 aggregated features & 0.2819 & 0.2493 & 0.2665 & 0.3242 & 0.2805 ± 0.0278 \\
 & (mean, std, count) & & & & & \\
\hline
Variation 1 & Dominant beacon & 0.3009 & 0.2621 & 0.2634 & 0.3327 & 0.2898 ± 0.0293 \\
 & features (top 3) & & & & & \\
\hline
Variation 2 & Signal pattern-based & 0.2830 & 0.2486 & 0.2633 & 0.3455 & 0.2851 ± 0.0369 \\
 & relabeling [8] & & & & & \\
\hline
\textbf{DASEL} & \textbf{Proposed} & \textbf{0.5114} & \textbf{0.4207} & \textbf{0.4340} & \textbf{0.4082} & \textbf{0.4438 ± 0.0295} \\
 & \textbf{sequential framework} & & & & & \\
\hline
\end{tabular}
\end{table*}
